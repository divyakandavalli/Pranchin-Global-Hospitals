        ...
        <div className="md:w-[55%] space-y-10">
          ...
          {(s.overview || s.moreoverview) && (
            <div>
              ...
              {s.overview && <p className="text-[#2f373e] text-[16px] xl:text-[18px] leading-relaxed">{s.overview}</p>}
              {s.moreoverview && <p className="text-[#2f373e] text-[16px] xl:text-[18px] mt-4 leading-relaxed">{s.moreoverview}</p>}
            </div>
          )}
          ...
          {(s.diagnostics || s.diagnosticsDescription) && (
            <div>
              {s.diagnosticsTitle && (
                <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.diagnosticsTitle}</h3>
              )}

              {/* Standalone diagnosticsDescription (used in Spine Surgery) */}
              {s.diagnosticsDescription && (
                <p className="text-[#2f373e] text-[16px] leading-relaxed mb-4">{s.diagnosticsDescription}</p>
              )}

              {s.diagnosticsDescription2 && <p className="text-[#2f373e] mb-4">{s.diagnosticsDescription2}</p>}
              {s.diagnosticssubtitle && <p className="text-[#2f373e] mb-4">{s.diagnosticssubtitle}</p>}
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-x-10 gap-y-3 text-[#2f373e] text-[16px]">
                  {s.diagnostics.map((item, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <DoneIcon fontSize="small" className="text-[#094ca0] mt-1 flex-shrink-0" />
                      <span>{item}</span>
                    </div>
                  ))}
                </div>
          ...
          {s.procedures && (
            <div>
              {s.proceduresTitle && (
                <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.proceduresTitle}</h3>
              )}
              {s.proceduresDescription && (
                <p className="text-[#2f373e] text-[16px] mb-4">{s.proceduresDescription}</p>
              )}
              {s.proceduresSubTitle && (
                <p className="font-medium text-[#2f373e] mb-4">{s.proceduresSubTitle}</p>
              )}
              ...
              {s.proceduresNote && (
                <p className="text-[#2f373e] text-[16px] mt-5 italic">{s.proceduresNote}</p>
              )}
            </div>
          )}

          {/* NEUROLOGY - conditionstreatedtitle */}
          {s.conditionstreatedtitle && (
            <div>
              <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.conditionstreatedtitle}</h3>
              {s.conditionstreateddesc && <p className="text-[#2f373e] mb-4">{s.conditionstreateddesc}</p>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {s.Majorconditions.map((item, i) => (
                <div key={i} className="bg-[#f5f7fa] p-6 rounded-2xl">
                  <h4 className="font-semibold text-[18px] mb-2">{item.title}</h4>
          ...
          {/* RHEUMATOLOGY - conditionsTreated Array */}
          {s.conditionsTreated && (
            <div>
              {s.extraSectionTitle && (
                <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.extraSectionTitle}</h3>
              )}
              {s.extraSectionContent && <p className="text-[#2f373e] mb-4">{s.extraSectionContent}</p>}
              ...
              {s.extraSectionNote && <p className="text-[#2f373e] mt-4">{s.extraSectionNote}</p>}
            </div>
          )}

          {/* GENERAL EXTRA SECTION */}
          {s.extraSectionTitle && !s.conditionsTreated && (
            <div>
              <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.extraSectionTitle}</h3>
              {s.extraSectionContent && (
                <p className="text-[#2f373e] text-[16px] leading-relaxed">{s.extraSectionContent}</p>
              )}
            </div>
          )}
          ...
          {s.screeningTitle && (
            <div>
              <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.screeningTitle}</h3>
              {s.screeningContent && <p className="text-[#2f373e] mb-6">{s.screeningContent}</p>}

              {s.packages && s.packages.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {s.packages.map((pkg, i) => (
                    <div key={i} className="flex justify-between items-center bg-[#c8dded] p-6 rounded-2xl">
                      <div>
          ...
          {(s.technologies || s.technologiesNote) && (
            <div>
              {s.technologiesTitle && (
                <h3 className="text-[24px] xl:text-[32px] font-semibold mb-4">{s.technologiesTitle}</h3>
              )}
              ...
              {/* technologiesNote - Now properly shown for Spine Surgery, Rheumatology, etc. */}
              {s.technologiesNote && (
                <p className="text-[#2f373e] text-[16px] leading-relaxed mt-6">{s.technologiesNote}</p>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-8">
            {images.map((img, i) => (
              <div key={i} className="rounded-3xl overflow-hidden shadow-md group">
                <img
                  src={img}
         ...
         <div className="md:w-[35%] relative my-10 xl:my-0">
          <div className="sticky top-24 space-y-6 bg-[#c8dded] rounded-[20px] p-8">
            ...
            <div className="bg-white rounded-xl overflow-hidden shadow-md">
              <img src={image3} alt="doctors" className="w-full h-auto object-cover" />
            </div>
